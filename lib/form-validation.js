(function (global, factory) {
  ...
  var dataPrefix = "data-".concat(vl.ns),
      ...
      fvNumericalityOnlyIntegerAtt = "".concat(dataPrefix, "numericality-only-integer"),
      fvNumericalityGreaterThanAtt = "".concat(dataPrefix, "numericality-greater-than"),
      fvNumericalityGreaterThanOrEqualToAtt = "".concat(dataPrefix, "numericality-greater-than-or-equal-to"),
      fvNumericalityLessThanAtt = "".concat(dataPrefix, "numericality-less-than"),
      fvNumericalityLessThanOrEqualToAtt = "".concat(dataPrefix, "numericality-less-than-or-equal-to"),
      fvValidatorOptions = {
    fullMessages: false
  },
  ...
  /**
   * Validators
   * data, rrn, uuid, phone, iban, multiselect, numericality
   */
  ...
  var validationJsNumericality = validate.validators.numericality.bind(validate);
  validate.validators.numericality = function (value, options, attribute, attributes, globalOptions) {
    const delocalisedValue = value != undefined ? value.replace(",", ".").replace(" ", "") : value;
    return validationJsNumericality(delocalisedValue, options, attribute, attributes, globalOptions);
  };

  var _buildNumericalityValidationConfig = function _buildNumericalityValidationConfig(field, obj) {
    return {
      message: obj.errorMessage,
      onlyInteger: field.getAttribute(fvNumericalityOnlyIntegerAtt) != undefined,
      greaterThan: parseFloat(field.getAttribute(fvNumericalityGreaterThanAtt)),
      greaterThanOrEqualTo: parseFloat(field.getAttribute(fvNumericalityGreaterThanOrEqualToAtt)),
      lessThan: parseFloat(field.getAttribute(fvNumericalityLessThanAtt)),
      lessThanOrEqualTo: parseFloat(field.getAttribute(fvNumericalityLessThanOrEqualToAtt))
    }
  }
  ...
  var _buildFormvalidationConfig = function _buildFormvalidationConfig(field, obj) {
    ...
    switch (obj.validationType) {
      ...
      case 'numericality':
        obj.validationConstraints.numericality = _buildNumericalityValidationConfig(field, obj);
        break;
    }
  ...
  var _showSuccessForInput = function _showSuccessForInput(input, errors) {
    if (!errors) {
      var isSelect = input.getAttribute(fvValidationType) === 'multi-select' || input.getAttribute(fvValidationType) === 'select';
